module.exports = {
    ...
    define: {
        parse: function (text) {
            ...
            var matches = text.match(/^\/\/\#(define) ([a-zA-Z0-9_]+)(.*)$/);
            if (matches) {
    ...
    undef: {
        parse: function (text) {
            //#undef TABLE_SIZE
            var matches = text.match(/^\/\/\#(undef) ([a-zA-Z0-9_]+)(.*)$/);
            if (matches) {
                if (matches.length == 4) {
                    return [matches[1], matches[2], matches[3]];
                }
    ...
    include: {
        parse: function (text) {
            ...
            var matches = text.match(/^\/\/\#(include) ([a-zA-Z0-9_\.\/]+)$/);
            if (matches) {
                return [matches[1], matches[2]];
            }
            return false;
    ...
    ifdef: {
        parse: function (text) {
            //#ifdef TABLE_SIZE
            var matches = text.match(/^\/\/\#(ifdef) ([a-zA-Z0-9_]+)$/);
            if (matches) {
                return [matches[1], matches[2]];
            }
            return false;
    ...
    ifndef: {
        parse: function (text) {
            //#ifndef TABLE_SIZE
            var matches = text.match(/^\/\/\#(ifndef) ([a-zA-Z0-9_]+)$/);
            if (matches) {
                return [matches[1], matches[2]];
            }
            return false;
    ...
    macro: {
        parse: function (text) {
            ...
            // /*#macro getSocket(number,second){
            // /*#macro getSocket(){
            var matches = text.match(/^\/\*#(macro) ([a-zA-Z0-9_]+) *\(([a-zA-Z0-9_ ,]*)\) *\{$/);
            if (matches) {
                return [matches[1], matches[2], matches[3]];
            }
            return false;
